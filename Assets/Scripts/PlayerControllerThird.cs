...
public class PlayerControllerThird : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        x = rightStick.x;
        y = rightStick.y;
        move = (x * moveTarget.right.normalized + y * moveTarget.forward.normalized ) * moveSpeed;
    ...
    void FixedUpdate()
    {
        transform.Rotate(new Vector3(0, leftStick.x, 0) * angleSpeed);
        transform.Translate(move, Space.World);
    }
}
